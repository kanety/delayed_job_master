  ...
  module Master
    class JobChecker
      ...
      def check(databases)
        ...
        check_current_jobs(databases)
        check_next_jobs(databases)
      rescue => e
        @master.logger.warn { "#{e.class}: #{e.message}" }
        @master.logger.debug { e.backtrace.join("\n") }
      end

      def check_current_jobs(databases)
        jobs = find_current_jobs(databases)
        jobs.each do |job|
      ...
      def find_current_jobs(databases)
        free_settings = count_free_settings
        free_settings.map do |setting, free_count|
          @job_finder.call(databases, setting, free_count)
        end.flatten
      end
      ...
      def check_next_jobs(databases)
        jobs = @job_finder.next_jobs(databases)
        jobs.each do |job|
